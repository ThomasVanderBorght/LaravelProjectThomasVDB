<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <select name="privacy_mode" id="privacy_mode" class="mt-1 block w-full">
                <option value="1" {{ $user->privacy_mode ? 'selected' : '' }}>Private</option>
                <option value="0" {{ !$user->privacy_mode ? 'selected' : '' }}>Public</option>
            </select>
            <x-input-error class="mt-2" :messages="$errors->get('privacy_mode')" />
        </div>
